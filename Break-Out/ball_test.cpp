...
void main_screen()
{
	string game_start, game_exit, control_key, select, ss;
	game_start = "start";
	...
	ss = "ranking";
	const unsigned char* u = (unsigned char*)ss.c_str();
	if (select_num == 2)
		glColor3f(1.0f, 0.0f, 0.0f);
	else
		glColor3f(1.0f, 1.0f, 1.0f);
	...
	if (select_num == 3)
		...
		glColor3f(1.0f, 1.0f, 1.0f);
	glRasterPos2i(width / 2, height / 2 - 150);
	...
	if(select_num == 0)
		...
		glRasterPos2i(width / 2 - 40, height / 2 - 150);
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, t);
}
...
void game_over()
{
	if (ball.pos.y <= 0 || (field_shape == 4 &&(ball.pos.x < 0 || ball.pos.x > width || ball.pos.y > height)))
	{
		...
		if (field_shape == 4 && !input_flag && field.stage != 1)
		{
...
void ranking()
{
	...
	const unsigned char* a = reinterpret_cast<const unsigned char*>("    rectangle");
	glRasterPos2i(width + 10, height / 10 * 9 + 30);
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, a);
	for (int i = 0; i < 3; i++)
	{
		if(rect.rank.record[i] == 10000)
		{
			for (int j = i; j < 3; j++)
			{
				s = to_string(i + 1) + ". 00 : 00 uesr";
				const unsigned char* a = (unsigned char*)s.c_str();
				glRasterPos2i(width + 10, height / 10 * 9 - 30 * i);
				glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, a);
			}
		}
	...
	glPopMatrix();
	glPushMatrix();
	glTranslatef(-550.0, -300.0, 0.0);
	const unsigned char* b = reinterpret_cast<const unsigned char*>("    rhombus");
	glRasterPos2i(width + 10, height / 10 * 9 + 30);
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, b);
	for (int i = 0; i < 3; i++)
	{
		if (rhombus.rank.record[i] == 10000)
		{
			for (int j = i; j < 3; j++)
			{
				s = to_string(i + 1) + ". 00 : 00 uesr";
				const unsigned char* t = (unsigned char*)s.c_str();
				glRasterPos2i(width + 10, height / 10 * 9 - 30 * i);
				glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, t);
			}
		}
	...
	const unsigned char* c = reinterpret_cast<const unsigned char*>("    circle");
	glRasterPos2i(width + 10, height / 10 * 9 + 30);
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, c);
	for (int i = 0; i < 3; i++)
	{
		if (circle.rank.record[i] == 10000)
		{
			for (int j = i; j < 3; j++)
			{
				s = to_string(i + 1) + ". 00 : 00 uesr";
	...
	const unsigned char* d = reinterpret_cast<const unsigned char*>("    stage");
	glRasterPos2i(width + 10, height / 10 * 9 + 30);
	glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, d);
	for (int i = 0; i < 3; i++)
	{
		if (field.rank.stage[i] == 1)
		{
			for (int j = i; j < 3; j++)
			{
				s = to_string(i + 1) + ". stage 1 user";
				const unsigned char* t = (unsigned char*)s.c_str();
				glRasterPos2i(width + 10, height / 10 * 9 - 30 * i);
				glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, t);
			}
...
void SpecialKey(int key, int x, int y)
{
	if (!main_num)
	{
		if (0 <= select_num && select_num <= 3)
		{
			switch (key)
			{
			case GLUT_KEY_UP: if(select_num != 0) select_num--; break;
			case GLUT_KEY_DOWN: if (select_num != 3) select_num++; break;
			}
		}
	}
	else
...
void MyMouse(int button, int state, int x, int y)
{
		...
		|| game_mode == 7 || couple_game_mode == 2 || couple_game_mode == 4 || couple_game_mode == 5) && x <= width)		// 3번, 4번 모드일때 맵을 그릴 수 있음
	{
		...
		else if (field_shape != 4 && (game_mode == 6 || game_mode == 7 || couple_game_mode == 5) && field.num / 2 > 4) return;
...
void bgm(int mode)
{
	...
	mciOpen.dwCallback = sizeof(MCI_OPEN_PARMS);

	mciOpen.dwCallback = sizeof(MCI_OPEN_PARMS);

	MCIERROR openError = mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE,
		(DWORD_PTR)(LPVOID)&mciOpen); // DWORD_PTR 사용

	if (openError != 0)
	{
		// 오류 처리: 파일 경로, 장치 타입 등을 확인
	...
	// **m_mciPlayParms의 크기 설정 (필수)**
	m_mciPlayParms.dwCallback = sizeof(MCI_PLAY_PARMS);

	// 참고: MCI_DGV_PLAY_REPEAT 플래그는 일반적으로 디지털 비디오용입니다.
	// 단순 오디오 반복(Loop)은 MCI_NOTIFY를 이용해 재생이 끝나면 다시 PLAY를 호출하는 방식으로 처리하는 것이 더 일반적입니다.

	MCIERROR playError = mciSendCommand(dwID, MCI_PLAY, MCI_DGV_PLAY_REPEAT, // play & repeat
		(DWORD_PTR)(LPVOID)&m_mciPlayParms);
}
